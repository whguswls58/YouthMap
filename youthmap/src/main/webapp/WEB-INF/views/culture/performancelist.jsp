...
<div class="cards">
  <c:forEach var="cul" items="${performancelist}">
    <div class="card">
      ...
      <!-- 2) 이미지에 링크 -->
      <a class="card-link" href="${pageContext.request.contextPath}/performancecont?page=${page}&con_id=${cul.con_id}">
        <div class="img-wrap">
   ...
   <c:if test="${!empty keyword }">
    ...
    <c:forEach var="i" begin="${startpage}" end="${endpage}">
      <c:choose>
        ...
        <c:otherwise>
          <a href="${pageContext.request.contextPath}/culturesearch?mainCategory=performance&search=${search}&keyword=${keyword}&page=${i}">${i}</a>
        </c:otherwise>
      </c:choose>
    </c:forEach>
    <c:if test="${endpage < pagecount}">
