...
<div id="card-container">
  ...
  <div class="pager">
   ...
   <c:if test="${!empty keyword }">
    <c:if test="${page > 1}">
      <a href="${pageContext.request.contextPath}/culturesearch?mainCategory=exhibition&search=${search}&keyword=${keyword}&page=1">«</a>
    </c:if>
    <c:if test="${startpage > 10}">
      <a href="${pageContext.request.contextPath}/culturesearch?mainCategory=exhibition&search=${search}&keyword=${keyword}&page=${startpage-10}">‹</a>
    </c:if>
    <c:forEach var="i" begin="${startpage}" end="${endpage}">
      <c:choose>
        <c:when test="${i == page}">
    ...
    <c:if test="${page < pagecount}">
      <a href="${pageContext.request.contextPath}/culturesearch?mainCategory=exhibition&search=${search}&keyword=${keyword}&page=${pagecount}">»</a>
    </c:if>
   </c:if>
  </div>
 </div>
